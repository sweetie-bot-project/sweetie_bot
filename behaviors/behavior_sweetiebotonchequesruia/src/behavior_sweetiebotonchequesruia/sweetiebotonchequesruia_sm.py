...
3. Perform random action and fall back to random head movements if no object.
	...
	def create(self):
		...
		_sm_watchonleapmotionobject_0 = ConcurrencyContainer(outcomes=['no_object', 'still_object', 'failed', 'to_close'], output_keys=['pose'], conditions=[
		...
		with _sm_watchonleapmotionobject_0:
			# x:135 y:135
		...
		# x:979 y:146
		_sm_switchevil_1 = OperatableStateMachine(outcomes=['finished'], input_keys=['be_evil'], output_keys=['be_evil'])

		with _sm_switchevil_1:
			# x:269 y:101
			...
			OperatableStateMachine.add('NormalEyes',
										TextCommandState(type='eyes/emotions', command='normal_look', topic=eyes_cmd_topic),
										transitions={'done': 'SetGood'},
										autonomy={'done': Autonomy.Off})

			# x:729 y:264
			OperatableStateMachine.add('RedEyes',
										TextCommandState(type='eyes/emotion', command='evil_look', topic=eyes_cmd_topic),
		...
		_sm_randheadmovements_2 = ConcurrencyContainer(outcomes=['object_detected', 'failed'], conditions=[
		...
		with _state_machine:
			...
			OperatableStateMachine.add('Brohoof',
										...
										transitions={'finished': 'CheckLeapMotionObject', 'unreachable': 'Greeting', 'failed': 'failed'},
			...
			OperatableStateMachine.add('CheckLeapMotionObject',
										...
										transitions={'no_object': 'RandHeadMovements', 'still_object': 'Play', 'moving_object': 'Play'},
										autonomy={'no_object': Autonomy.Off, 'still_object': Autonomy.Off, 'moving_object': Autonomy.Off},
										remapping={'pose': 'pose'})
