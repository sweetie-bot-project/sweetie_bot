#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""OpenMV face and eyes recognition JSON bridge"""
...
from face_recognition.msg import FaceTag, TagRect, FaceTags

JSON_MESSAGE_BUFFER_LIMIT = 100  # JSON message buffer limit

# Main function
if __name__ == '__main__':
    # Initialize the node, name it, set log verbosity level
    rospy.init_node('face_recognition', log_level=rospy.DEBUG)
    ...
    faces_publisher = rospy.Publisher('{0}/faces'.format(node_prefix), FaceTags, queue_size=10)
        ...
        try:
            ...
            if not serial_byte:  # No data in the serial port buffer (read timeout)
                ...
                if len(json_message_buffer):  # Not an empty JSON message
                    ...
                    try:
                        faces = json.loads(json_message_buffer)  # Decode the JSON message
                        #  rospy.logdebug("Decoded message from OpenMV: '%s'",
                        #    faces)

                        faces_message.header.stamp = rospy.Time.now()
                        ...
                        for face in faces:
                            ...
                            face_msg.face = TagRect(int(face['x']), int(face['y']), int(face['w']),
                                                    int(face['h']))
                            face_msg.eyes = [TagRect(int(eye['x']), int(eye['y']), int(eye['w']),
                                                     int(eye['h'])) for eye in face['eyes']]

                            faces_message.faces.append(face_msg)

                        faces_publisher.publish(faces_message)
