...
class Clock(input_module.InputModule):
    ...
    def __init__(self, name, config, agent):
        ...
        # create WMES
        self._now_id = agent.CreateFloatWME(self._sensor_id, "now", time_now)
        self._just_id = agent.CreateFloatWME(self._sensor_id, "just", time_now - self._just_delay)
        self._recently_id = agent.CreateFloatWME(self._sensor_id, "recently", time_now - self._recently_delay)
        self._long_time_id = agent.CreateFloatWME(self._sensor_id, "long-time", time_now - self._long_time_delay)

        # cycle counter
        self._cycle = 0
        self._cycle_id = agent.CreateIntWME(self._sensor_id, "cycle", self._cycle)
