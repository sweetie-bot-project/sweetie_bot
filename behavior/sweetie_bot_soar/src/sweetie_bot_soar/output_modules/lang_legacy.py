# Legacy CompleteRaw + regex/BERT lang-model backend, preserved from devel lang.py verbatim
# (class renamed LangModel -> LegacyLangModel; registration lives in lang.py dispatcher).
...
#
# WME helpers moved to wme_parsing.py (shared with the agent backend, R6);
# re-imported here so existing `from .lang_legacy import TalkEvent, ...` keeps working.
#

from .wme_parsing import (WMEParseError, GetChildValueAsType,  # noqa: F401,E402 - re-export
                          TalkEvent, Predicate)
...
class LegacyLangModel(OutputModule):
