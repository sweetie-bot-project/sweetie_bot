...
from animation_stored_trajectory_state import AnimationStoredJointTrajectoryState
...
class SweetieBotCompoundAction(EventState):
	...
	Actions are translated into other flexbe states. No AnimationStoredJointTrajectoryState and TextCommandState are supported.
	...
	-- type1		string				Type of the first action. None means no action. 'motion/joint_trajectory' maps to AnimationStoredJointTrajectoryState( trajectory_param='joint_trajectory/'+cmd, action_topic='motion/controller/joint_trajectory') Other values are converted to TextCommandState( type=type, cmd=cmd, topic='control').
		...
		for i, p in enumerate(params):
			...
			if p.type == None:
				...
				if p.type == 'motion/joint_trajectory':
					flexbe_state = AnimationStoredJointTrajectoryState(action_topic = 'motion/controller/joint_trajectory', trajectory_param = 'joint_trajectory/' +  p.cmd)
